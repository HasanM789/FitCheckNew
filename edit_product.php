...
// Get current sizes as array
$current_sizes = !empty($product['sizes']) ? explode(',', $product['sizes']) : ['S', 'M', 'L', 'XL'];
$current_size_type = $product['size_type'] ?? 'clothing';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_product'])) {
        ...
        if ($check !== false) {
            ...
            if (in_array($imageFileType, $allowed_types)) {
                if (move_uploaded_file($_FILES['product_image']['tmp_name'], $target_file)) {
                    ...
                    if (!empty($product['image_url']) && file_exists($product['image_url']) && $product['image_url'] != 'product.jpg') {
    ...
    if (!isset($error)) {
        $stmt = $conn->prepare("UPDATE products SET name = ?, description = ?, price = ?, category = ?, image_url = ?, sizes = ?, size_type = ?, stock = ? WHERE id = ?");
        $stmt->bind_param("ssdssssii", $name, $description, $price, $category, $image_url, $sizes, $size_type, $stock, $product_id);
...
<head>
    <meta charset="UTF-8">
    <title>Edit Product - FitCheck</title>
    <style>
        ...
        .btn-secondary { background: transparent; border: 1px solid #333; color: #fff; padding: 8px 20px; border-radius: 4px; text-decoration: none; font-size: 14px; display: inline-block; }
        ...
        .current-image { max-width: 200px; max-height: 200px; object-fit: contain; border-radius: 4px; margin: 10px 0; display: block; background: #0f1115; }
        ...
        .image-info { color: #94a3b8; font-size: 13px; margin: 5px 0 15px 0; }
        ...
        .image-preview { max-width: 200px; max-height: 200px; object-fit: contain; border-radius: 4px; margin: 10px 0; display: none; background: #0f1115; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <textarea name="description" required rows="3"><?php echo htmlspecialchars($product['description']); ?></textarea>
            ...
            <select name="category" required>
                <?php foreach ($categories as $cat): ?>
                    <option value="<?php echo $cat; ?>" <?php if($product['category'] == $cat) echo 'selected'; ?>><?php echo $cat; ?></option>
                <?php endforeach; ?>
            </select>
            
            <label>Size Type *</label>
            <select name="size_type" id="size_type" required>
                <option value="clothing" <?php if($current_size_type == 'clothing') echo 'selected'; ?>>Clothing (XS, S, M, L, XL, XXL)</option>
                <option value="shoes" <?php if($current_size_type == 'shoes') echo 'selected'; ?>>Shoes (36-46 EU)</option>
            </select>
            
            <label>Available Sizes</label>
            <div id="clothing_sizes" class="size-checkboxes" <?php if($current_size_type == 'shoes') echo 'style="display:none;"'; ?>>
                <?php foreach ($clothing_sizes as $size): ?>
                    <label>
                        <input type="checkbox" name="sizes[]" value="<?php echo $size; ?>" <?php if(in_array($size, $current_sizes)) echo 'checked'; ?>>
                        <?php echo $size; ?>
                    </label>
                <?php endforeach; ?>
            </div>
            
            <div id="shoe_sizes" class="size-checkboxes" <?php if($current_size_type == 'clothing') echo 'style="display:none;"'; ?>>
            ...
            <input type="number" name="stock" required min="0" value="<?php echo $product['stock'] ?? 0; ?>">
            
            <label>Current Image</label>
            <?php if (!empty($product['image_url']) && file_exists($product['image_url'])): ?>
                <img src="<?php echo $product['image_url']; ?>" class="current-image">
                <p class="image-info"><?php echo $product['image_url']; ?></p>
            <?php else: ?>
                <p class="image-info">No image uploaded</p>
            <?php endif; ?>
